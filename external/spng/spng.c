...
static int calculate_subimages(struct spng_ctx *ctx) {
  ...
  for (i = 0; i < 7; i++) {
    ...
    int ret =
        calculate_scanline_width(ihdr, sub[i].width, &sub[i].scanline_width);
    if (ret) return ret;
...
static unsigned get_best_filter(const unsigned char *prev_scanline,
  ...
  int32_t filter_scores[5] = {INT32_MAX, INT32_MAX, INT32_MAX, INT32_MAX,
                              INT32_MAX};
  ...
  for (i = 0; i < 5; i++) {
    ...
    if (choices & flag)
      sum = filter_sum(prev_scanline, scanline, scanline_width, bytes_per_pixel,
                       i);
    else
      continue;
...
static uint16_t sample_to_target(uint16_t sample, unsigned bit_depth,
  ...
  if (bit_depth == sbits) {
    ...
    sample =
        sample >> (bit_depth - sbits); /* Shift significant bits to bottom */
...
static int check_chrm_int(const struct spng_chrm_int *chrm_int) {
  ...
  if (chrm_int->white_point_x > spng_u32max ||
      chrm_int->white_point_y > spng_u32max || chrm_int->red_x > spng_u32max ||
      chrm_int->red_y > spng_u32max || chrm_int->green_x > spng_u32max ||
      chrm_int->green_y > spng_u32max || chrm_int->blue_x > spng_u32max ||
...
/* Returns non-zero for standard chunks which are stored without allocating
 * memory */
  ...
  while (!(ret = read_header(ctx))) {
    ...
    if (!memcmp(chunk.type, type_idat, 4)) {
      ...
      if (ctx->prev_was_idat) {
        ...
        return SPNG_ECHUNK_POS; /* IDAT chunk not at the end of the IDAT
                                   sequence */
    }
    ...
    } else /* Arbitrary-length chunk */
    {
      if (!memcmp(chunk.type, type_exif, 4)) {
        ...
        ctx->stored.exif = 1;
      } else if (!memcmp(chunk.type, type_iccp,
        ...
        ret = spng__inflate_stream(
            ctx, &ctx->iccp.profile, &ctx->iccp.profile_len, 0,
        ...
        void *buf = spng__realloc(ctx, ctx->text_list,
        ...
        if (!memcmp(chunk.type, type_text, 4)) {
          ...
          /* at least two 1-byte fields, two >=0 length strings, and one byte of
           * (compressed) text */
          ...
          term = memchr(data + language_tag_offset, 0,
          ...
          zlib_stream = memchr(data + translated_keyword_offset, 0,
        ...
        if (text->compression_flag) {
          ...
          ret = spng__inflate_stream(ctx, &text->text, &text->text_length, 1,
                                     zlib_stream, peek_bytes - text_offset);
          ...
          if (chunk.length > peek_bytes) {
            ret = read_chunk_bytes2(ctx, text->keyword + peek_bytes,
        ...
        /* chunk.length + sizeof(struct spng_splt) + splt->n_entries *
         * sizeof(struct spng_splt_entry) */
        if (increase_cache_usage(ctx, chunk.length + sizeof(struct spng_splt),
                                 1))
          return SPNG_ECHUNK_LIMITS;
        ...
        void *buf = spng__realloc(ctx, ctx->splt_list,
      ...
      } else /* Unknown chunk */
      {
        ...
        if (increase_cache_usage(
                ctx, chunk.length + sizeof(struct spng_unknown_chunk), 1))
          return SPNG_ECHUNK_LIMITS;
        ...
        void *buf =
            spng__realloc(ctx, ctx->chunk_list,
  ...
  while (ctx->state < SPNG_STATE_FIRST_IDAT ||
    ...
    if (!ret) {
      ...
      switch (ret) {
        ...
        case SPNG_ECHUNK_SIZE: /* size != expected size, SPNG_ECHUNK_STDLEN =
                                  invalid size */
        case SPNG_EDUP_PLTE:
        case SPNG_EDUP_CHRM:
        case SPNG_EDUP_GAMA:
...
static int read_scanline(spng_ctx *ctx) {
  ...
  ret = defilter_scanline(ctx->prev_scanline, ctx->scanline, scanline_width,
                          ctx->bytes_per_pixel, ri->filter);
  if (ret) return ret;
...
int spng_decode_scanline(spng_ctx *ctx, void *out, size_t len) {
  ...
  if (f.apply_trns)
    trns_row(out, scanline, trns_px, ctx->bytes_per_pixel, &ctx->ihdr, width,
             fmt);
  ...
  if (ret == SPNG_EOI) {
    if (ctx->cur_chunk_bytes_left) /* zlib stream ended before an IDAT chunk
                                      boundary */
...
int spng_decode_image(spng_ctx *ctx, void *out, size_t len, int fmt,
  ...
  if (ihdr->bit_depth == 16 &&
      fmt & (SPNG_FMT_RGBA8 | SPNG_FMT_RGB8)) { /* samples are scaled down by 8
                                                   bits in the decode loop */
    sb->red_bits -= 8;
    sb->green_bits -= 8;
    sb->blue_bits -= 8;
    sb->alpha_bits -= 8;
  ...
  if (f.indexed) {
    ...
    for (i = 0; i < 256; i++) {
      ...
      green =
          sample_to_target(ctx->plte.entries[i].green, 8, sb->green_bits, 8);
      blue = sample_to_target(ctx->plte.entries[i].blue, 8, sb->blue_bits, 8);
      alpha =
          sample_to_target(ctx->plte.entries[i].alpha, 8, sb->alpha_bits, 8);
  ...
  do {
    ...
    ret =
        spng_decode_row(ctx, (unsigned char *)out + ioffset, ctx->image_width);
  } while (!ret);
...
static int write_chunks_before_idat(spng_ctx *ctx) {
  ...
  if (ctx->stored.trns) {
    if (ctx->ihdr.color_type == SPNG_COLOR_TYPE_GRAYSCALE) {
      ...
      ret = write_chunk(ctx, type_trns, ctx->trns.type3_alpha,
...
static int encode_scanline(spng_ctx *ctx, const void *scanline, size_t len) {
  ...
  ret =
      write_idat_bytes(ctx, filtered_scanline - 1, scanline_width, Z_NO_FLUSH);
  if (ret) return encode_err(ctx, ret);
...
int spng_get_text(spng_ctx *ctx, struct spng_text *text, uint32_t *n_text) {
  ...
  for (i = 0; i < ctx->n_text; i++) {
    ...
    memcpy(&text[i].keyword, ctx->text_list[i].keyword,
...
int spng_set_text(spng_ctx *ctx, struct spng_text *text, uint32_t n_text) {
  ...
  for (i = 0; i < n_text; i++) {
    ...
    memcpy(text_list[i].user_keyword_storage, text[i].keyword,
